...
// El header se esconde al bajar y se queda quieto (no reaparece solo).
// Para traerlo de vuelta: el botón flotante ↑ lleva arriba del todo.
function setTopbar(hide){{
  if(hide===topbarHidden)return;
  topbarHidden=hide;
  topbar.style.marginTop=hide?(-topbar.offsetHeight-2)+'px':'0px';
  toTop.classList.toggle('show',hide);
}}
function watchScroll(win){{
  ...
  win.addEventListener('scroll',()=>{{
    ...
    const y=win.scrollY;
    if(y<=20)setTopbar(false);        // arriba del todo → header visible
    else if(y>140)setTopbar(true);    // ya bajaste → header escondido y quieto
  }},{{passive:true}});
}}
...
function goTop(){{
  const p=panels.find(x=>x.classList.contains('active'));
  if(p&&p.contentWindow){{
    try{{p.contentWindow.scrollTo({{top:0,behavior:'smooth'}});}}catch(e){{p.contentWindow.scrollTo(0,0);}}
  }}
  setTopbar(false);
}}
