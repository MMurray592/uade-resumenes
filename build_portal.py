...
function watchScroll(win){{
  ...
  let upAcc=0;
  const UP_THRESHOLD=90;  // px de desliz hacia arriba antes de mostrar el header
  win.addEventListener('scroll',()=>{{
    ...
    const dy=y-lastY;
    lastY=y;
    if(y<=20){{upAcc=0;setTopbar(false);setStickies(doc,false);return;}}
    if(dy>5){{upAcc=0;setTopbar(true);setStickies(doc,true);}}
    else if(dy<0){{
      upAcc+=-dy;
      if(upAcc>=UP_THRESHOLD){{upAcc=0;setTopbar(false);setStickies(doc,false);}}
    }}
  }},{{passive:true}});
}}
